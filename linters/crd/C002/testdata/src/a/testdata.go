...
type Bad struct {
	...
	MandatoryField string `json:"mandatoryField,omitempty"` // want `C002: Field 'MandatoryField' has both a 'Required' kubebuilder marker and an 'omitempty' tag. Only one of them should be used`

	// MandatoryField2 wants to be mandatory but omitempty makes it optional
	// +kubebuilder:validation:Required
	MandatoryField2 string `json:"mandatoryField2,omitempty"` // want `C002: Field 'MandatoryField2' has both a 'Required' kubebuilder marker and an 'omitempty' tag. Only one of them should be used`

	//+kubebuilder:validation:Required
	// MandatoryField3 wants to be mandatory but omitempty makes it optional
	MandatoryField3 string `json:"mandatoryField3,omitempty"` // want `C002: Field 'MandatoryField3' has both a 'Required' kubebuilder marker and an 'omitempty' tag. Only one of them should be used`
}
